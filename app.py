...
def dashboard():
    ...
    summary_today = db.user_summary(conn, today_start)
    events = db.recent_events(conn, today_start, limit=100)
    poll = db.latest_poll(conn)

    total_alerts = sum(r["alert_count"] for r in summary_today)
    total_inactive = sum(r["total_inactive_seconds"] for r in summary_today)
    monitored = sum(1 for r in summary_today if not r["ignored"])
